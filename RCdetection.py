...
def geoCodeTiff_new(base_dir, master):
    ...
    range_dec = cf.get("other", "range_dec")
    azimuth_dec = cf.get("other", "azimuth_dec")
    width_ = cf.get("other", "num_rng_bins")
    height_ = cf.get("other", "num_valid_az")

    # 经纬度重采样
    command = "D:/cygwin64/project/sample.exe " + base_dir + " " + str(height) + " " + range_dec + " " + azimuth_dec
    os.popen(command).readlines()

    # 文件格式重新命名
    os.remove(out_filename_lon)
    os.remove(out_filename_lat)

    os.rename(compose_dir + r'\latitude_osp_out', out_filename_lat)
    os.rename(compose_dir + r'\longitude_osp_out', out_filename_lon)
    ...
    cf = configparser.ConfigParser()
    cf.read(inifile)
    cf.set('PARAMETER-PRE', 'width', str(float(width_)))
    cf.set('PARAMETER-PRE', 'height', str(float(height_)))
...
class Time_thread(QThread):
    ...
    def __init__(self):
        ...
        self.out_cf_name = filepath2ini + '/filter.ini'

    def run(self):
        global method

        #滤波预处理
        self.filter_pre_process()
    ...
    def filter_pre_process(self):
        cmd = "D:/cygwin64/project/cmd/for_rc_pre_filter.bat  " + self.out_cf_name  #cmd执行滤波预处理
        os.popen(cmd).read()

    def NL_filter(self):
        # 扩展filter.ini参数文件
        config = configparser.ConfigParser()
        config.read(self.out_cf_name)
        ...
        config.write(open(self.out_cf_name, "w"))  # 写入batch.ini文件

        width = config.get("PARAMETER-PRE", "width")
        height = config.get("PARAMETER-PRE", "height")

        cf = configparser.ConfigParser()
        cf.read(filepath2ini + '/config.ini')
        width_ = cf.get("other", "num_rng_bins")
        height_ = cf.get("other", "num_valid_az")

        cf.read(self.out_cf_name)
        cf.set('PARAMETER-PRE', 'width', str(float(width_)))
        cf.set('PARAMETER-PRE', 'height', str(float(height_)))
        cf.write(open(self.out_cf_name, "w"))

        # 调用cmd处理
        cd_command = "cd D:/cygwin64/project/rc/ & d:"  # cd到目标文件夹
        cmd = cd_command + " & " + "D:/cygwin64/project/rc/rs_stage2.exe  " + self.out_cf_name  # cmd执行滤波预处理
        os.system('cd D:/cygwin64/project/rc/')
        tmp = os.popen(cmd).read()

        cf.read(self.out_cf_name)
        cf.set('PARAMETER-PRE', 'width', str(float(width)))
        cf.set('PARAMETER-PRE', 'height', str(float(height)))
        cf.write(open(self.out_cf_name, "w"))
    ...
    def Lee_filter(self):
        global output_path, filter_window, image_name_list
        # 调用Lee滤波函数处理
        # 定义文件路径
        pgmDir_list = []
        datDir_list = []
        ...
        for imagePath in image_name_list_new_2:
            ...
            img_filter = self.Lee_filter_output(img, filter_window_float)
            ...
            im.convert('L').save(
                outPath_temp + out_fileName.split('.')[0] + '_final.pgm')  # 如果是rgb图，要转为单通道的灰度图；如果是灰度图，那么去掉convert，保持灰度图
    ...
    def Lee_filter_output(self, img, size):
...
class Ui_Dialog(QtWidgets.QDialog):
    ...
    def create_tsx_input(self, file_path):  # 筛选terrasar数据cos和xml文件，生成input.txt文件
        ...
        input_txt.close()
        return True
    def create_gf3_input(self, file_path):  # 筛选terrasar数据cos和xml文件，生成input.txt文件
        contents = []
        input_txt = open(self.input_txt_path, "w")
        for maindir, subdir, file_name in os.walk(file_path):
            if maindir != file_path:
                for file in file_name:
                    if '.xml' in file:
                        xml_path = os.path.join(maindir, file)
                    else:
                        cos_path = os.path.join(maindir, str(file))
                mode = file.split("_")[2]
                self.set_config('other', 'gf3mode',mode)
                #filepath = os.path.abspath(os.path.join(cos_path, "../.."))
                date = os.path.basename(file).split('_')[6][:8]
                self.image_date.append(date)
                #xml_file = os.path.basename(filepath) + '.xml'
                #xml_path = os.path.join(filepath, xml_file)
                Elements = xml.dom.minidom.parse(xml_path).documentElement
                orbitdirection = Elements.getElementsByTagName('Direction')[0].firstChild.data
                polar = Elements.getElementsByTagName('lookDirection')[0].firstChild.data
                name = "gf3" + '_' + str(date) + '_' + polar.lower() + '_' + orbitdirection[0].lower() + '_' + mode
                content = [name, xml_path, cos_path]
                contents.append(content)
        files = sorted(contents, key=lambda y: y[0][4:12])
        for file in files:
    ...
    def pushbutton_1_click(self):  # 导入数据
        ...
        if flag == False:
            ...
            while raw_paths == "":
                QMessageBox.warning(self, "Warning", "影像数据选择错误", QMessageBox.Yes | QMessageBox.Yes)
                raw_paths = QFileDialog.getExistingDirectory(self, "选取影像数据文件夹", "D:/workspace/")

            dir_grd, filetype = QFileDialog.getOpenFileName(self, "选取dem文件(*.grd)", "D:/workspace/","Grd Files (*.grd)")
            while dir_grd == "" or filetype != "Grd Files (*.grd)":
                QMessageBox.warning(self, "Warning", "dem文件选择错误", QMessageBox.Yes | QMessageBox.Yes)
                dir_grd, filetype = QFileDialog.getOpenFileName(self, "选取dem文件(*.grd)", "D:/workspace/", "Grd Files (*.grd)")
            ...
            if self.data_type == 'GF3': #处理gf3号卫星数据
                flag = self.create_gf3_input(raw_paths)
        if flag == True:
            print("数据导入成功")
    ...
    def pushbutton_2_click(self):  # 影像配准
        ...
        if flag == True:
            ...
            time_used = np.round(((time.time() - start) / 60), 2)
            win32api.MessageBox(0, "影像配准完成! 耗时" + str(time_used) + "分 ", "进度说明", win32con.MB_OK)
        else:
            print("影像配准失败")
    ...
    def pushbutton_3_click(self):  # 影像裁剪
        ...
        if flag == False:
            ...
            code_path = "D:\cygwin64\project\select_roi"
            now_path = os.getcwd()
            os.chdir(code_path)
            select_roi = "select_roi.exe " + self.workspace
            ...
            # 设置多视参数
            my3 = child3Window()
            my3.child3_ui.lineEdit_2.setText("1")
            my3.child3_ui.lineEdit_3.setText("1")
            my3.child3_ui.lineEdit_3.setEnabled(False)
            my3.exec_()

            range_dec = my3.child3_ui.lineEdit_2.text()
            azimuth_dec = my3.child3_ui.lineEdit_3.text()
            self.set_config("other", "range_dec", str(range_dec))
            self.set_config("other", "azimuth_dec", str(azimuth_dec))

            if (range_dec != "1" or azimuth_dec != "1"):
                num_valid_az = int(np.floor(int(self.get_config("other", "num_valid_az")) / int(azimuth_dec)))
                num_rng_bins = int(np.floor(int(self.get_config("other", "num_rng_bins")) / int(range_dec)))
                self.set_config("other", "num_valid_az", str(num_valid_az))
                self.set_config("other", "num_rng_bins", str(num_rng_bins))
            ...
            for index in range(len(lines)):
                ...
                command_multi_look.append("D:/cygwin64/project/cmd/multi_look.bat " + self.workspace + " " + date)
        ...
        if flag == True:
            ...
            print("影像裁剪完成")
            time_used = np.round(((time.time() - start) / 60), 2)
            win32api.MessageBox(0, "影像裁剪完成! 耗时" + str(time_used) + "分 ", "进度说明", win32con.MB_OK)

    def pushbutton_4_click(self):
        self.workspace = self.lineEdit.text()

        # 运行前清理之前结果文件
        if os.path.exists(self.workspace + "/composed"):
            shutil.rmtree(self.workspace + "/composed")
        if os.path.exists(self.workspace + "/filter"):
            shutil.rmtree(self.workspace + "/filter")
        if os.path.exists(self.workspace + "/stretch"):
            shutil.rmtree(self.workspace + "/stretch")
        if os.path.exists(self.workspace + "/filter.ini"):
            os.remove(self.workspace + "/filter.ini")
        print('滤波!')
...
class child3Window(QDialog):  # 设置视数假面
    def __init__(self):
        QDialog.__init__(self)
        self.child3_ui = select_multilook.Ui_Dialog()
        self.child3_ui.setupUi(self)
...
class child6Window(QDialog):
    def __init__(self):
        ...
        self.child6_ui.Btn_OK.clicked.connect(self.update_ui)
        self.child6_ui.Btn_OK.clicked.connect(self.select)

        self.child6_ui.radio_filter_Nl.clicked.connect(self.NL_click)
        self.child6_ui.radio_filter_Nl_2.clicked.connect(self.Lee_click)

        self.timethread = Time_thread()

        self.timethread.sinout.connect(self.close)

        global batchfile
        global filepath2ini
        global image_name_list_new, search_radius, adjacent_radius, gaussian_para, output_path

        self.set_params_window()
        self.set_config_for_pre_filter()

    def set_params_window(self):
        # 读取参数并写入界面
        #  读取长、宽（region_cut）
    ...
    # 动态显示运行过程
    def update_ui(self):
        self.child6_ui.Btn_OK.setVisible(False)
        self.child6_ui.Btn_Cancel.setVisible(False)
        self.child6_ui.progressBar.setVisible(True)
        self.child6_ui.label_2.setVisible(True)

    # 返回初始界面
    def back_ui(self):
        self.child6_ui.Btn_OK.setVisible(True)
        self.child6_ui.Btn_Cancel.setVisible(True)
        self.child6_ui.progressBar.setVisible(False)
        self.child6_ui.label_2.setVisible(False)

    def set_config_for_pre_filter(self):
        ...
        if os.path.exists(out_cf_name):
            os.remove(out_cf_name)
    ...
    def NL_click(self):
        global method
        self.child6_ui.lineEdit.setVisible(False)
        self.set_config_for_pre_filter()
        my7 = child7Window()
        my7.exec_()
    ...
    # lee滤波
    def Lee_click(self):
        self.child6_ui.lineEdit.setVisible(True)
        self.child6_ui.lineEdit.setText("请输入Lee滤波窗口")
        self.set_config_for_pre_filter()

    # 判断用户所选滤波方式
    def select(self):
        global method
        if self.child6_ui.radio_filter_Nl.isChecked():
            method = "NL"
            self.timethread.start()
        if self.child6_ui.radio_filter_Nl_2.isChecked():
            self.child6_ui.lineEdit.setVisible(True)
            if self.set_lee_config():
                method = "Lee"
                self.timethread.start()
            else:
                self.back_ui()
    ...
    # 设置lee滤波相应参数
    def set_lee_config(self):
        global output_path, filter_window, image_name_list
        stretch_path = filepath2ini + '\\stretch'
        output_path = filepath2ini + '\\'
        ...
        if (os.path.exists(stretch_path)):
            ...
            print("暂未进行滤波预处理")
            lines = open(filepath2ini + "/data/input.txt").readlines()
            master_tif_path = stretch_path + "/" + lines[0].split("_")[1] + ".tif"
            slave_tif_path = stretch_path + "/" + lines[1].split("_")[1] + ".tif"
            image_name_list = master_tif_path + '\n' + slave_tif_path

        # 设置滤波默认参数
        filter_window = self.child6_ui.lineEdit.text().replace(" ","")
        if filter_window == "" or filter_window == "请设置Lee滤波窗口":
            QMessageBox.question(self, '错误信息', "请输入正确的窗口大小", QMessageBox.Yes | QMessageBox.Yes)
            return False
...
class child7Window(QDialog):
    def __init__(self):
        ...
        self.child7_ui.pushButton.clicked.connect(self.Btn_OK)  #OK
...
class child_sample_Window(QDialog):
    ...
    def Btn_OK_click(self):
        ...
        res = QInputDialog.getText(self, "样本生成参数设置", "是否生成外接矩形 Y-是 N-否", text = 'Y')[0]
        if res == 'Y':
            flag_outline = 'True'
        else:
            flag_outline = 'False'
        config.set("PARAMETER-PRE", 'flag_outline', flag_outline)
        config.write(open(out_cf_name, "w"))  # 写入batch.ini文件
    ...
    def classification(self):
        ...
        # shp_file = filepath2ini + '/extraction/RC_vec.shp'
        # cmd = "D:/cygwin64/project/cmd/merge_shp.bat  " + shp_file
        # os.popen(cmd).read()
